    ...
    <div
              ...
              <div className='flex flex-col space-y-4 p-4'>
                {displayMessages.map((message, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={cn('flex gap-3', message.role === 'user' ? 'justify-end items-start' : 'justify-start items-start')}
      ...
      <form ref={inputAreaRef} onSubmit={handleSubmit} className='flex gap-2 p-4 border-t bg-background/50 items-end'>
        <Textarea
          ...
          rows={1} />
        <Button type='submit' size='icon' disabled={!input.trim() || isLoading} className='shrink-0 h-9.5 w-9.5'>
